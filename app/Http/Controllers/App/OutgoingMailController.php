...
class OutgoingMailController extends Controller
{
	...
	public function store(Request $r)
	{
		...
		foreach ($images as $img) {
			...
			rename(public_path('assets/tesseract').'/'.Auth::user()->_id.'/'.$img, public_path('assets/app/img/outgoing_mail').'/'.$nm_file);
			$files[] = $nm_file;
		}
	...
	public function move($id)
	{
		...
		for ($i=0; $i < count($files) ; $i++) { 
			copy(public_path('assets/app/img/outgoing_mail').'/'.$files[$i], public_path('assets/tesseract').'/'.Auth::user()->_id.'/'.$filename[$i].'.'.$ext[$i]);
		}
	...
	public function update($id, Request $r)
	{
		...
		for ($i=0; $i < count($old) ; $i++) { 
			unlink(public_path('assets/app/img/outgoing_mail').'/'.$old[$i]);
		}
