...
class IncomingMailController extends Controller
{
	...
	public function totalStorageCompany()
	{
		$idCompany = Auth::user()->_id;
		$users = Archieve::select('files')->where('id_company', Auth::user()->id_company)->first();
		$imageLocation = public_path('files/'.$idCompany.'/incoming_mail/'.$users['files'][0]);
	...
	public function upload(Request $r)
	{
		...
		if ($check == 'pdf') {
			...
			// For Windows
			$output = exec('tesseract "'.$image.'" "'.$result.'" -l');

			// For Mac
			$output = exec('/usr/bin/tesseract "'.$image.'" "'.$result.'" -l');
